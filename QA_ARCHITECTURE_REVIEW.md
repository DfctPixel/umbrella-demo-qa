...
**CI completion:** the same manual run completed with failure. Lint and TypeScript checks passed. UI remained **3 failed, 5 passed**; the chart diagnostic confirms that the actual feed uses `periodGranLevel: "day"`, `costType`, and `groupBy: ["service", "usagedate"]`, not the test's assumed `granularity`/`metrics` shape. The CSV tests still derive duplicate identifier `5.0`. The API suite still returns `400` for the unchanged `plain-sub-users` request and related unscoped requests.

5. **P1 — remove each separately verified obsolete expected-failure marker.** CI now reports unexpected passes for reversed anomaly dates (`boundary-edge.spec.ts:70`), recommendation `pageNumber: 0` (`:92`), `pageSize: 0` (`:102`), negative page number (`:126`), and invalid anomaly `startDate` (`error-paths.spec.ts:85`). The earlier empty-page-number marker was correctly removed, but these five remaining `test.fail()` markers keep a verified behavior red. Remove only those individual markers while preserving their exact `400` assertions and issue history.

**Updated DeepSeek handoff:** Use the observed CAUI structure to implement a strict, sanitized chart contract; fix the UI header/cell extraction before any key comparison; and replace the generic API contract endpoint list with request builders that reproduce the required endpoint context. Remove the five stale per-test expected-failure markers. Dispatch one manual CI run after all changes are committed.
